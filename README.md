...
```bash
s                  # alias global instalado por el .deb en /usr/local/bin/s
...
| `Enter` / `→` / `l` | En el panel izquierdo: entrar al host (foco a la derecha) o expandir/colapsar grupo. **Sobre un servicio (panel derecho): ver su historial** (cortes y regresos, como hacer clic en el servicio en la web) |
| `←` / `h` | Volver / cerrar el historial / colapsar grupo |
...
| `H` | Atajo de historial (7 días, cambios de estado): del servicio seleccionado si estás en el panel derecho, del host si no. Repetir `H` alterna servicio → host → cerrar |
| `r` | Refrescar ahora |
| `q` | Salir |

> El historial muestra sólo las **transiciones** de estado (rojo↔verde…) de los últimos 7 días. Un servicio que nunca cambió de color en ese período aparece como "(sin eventos en el período)".
...
| Tag `v*` (ej: `v3.7.6`) | Build + crea un **GitHub Release** con los `.deb` adjuntos |
...
**TUI: dos bugs encontrados al revisar el código nuevo (v3.6.6–v3.7.5)**
- **I/O de disco por segundo**: el historial inline agregado en v3.7.4 se releía en **cada repintado** (la pantalla se repinta 1 vez por segundo por el timeout de `getch`), no en cada refresco. En hosts con archivos de historial grandes eso significaba leer y parsear varios MB por segundo: medido **125 ms por repintado** en el peor host real (5,6 MB). Ahora el historial se cachea por `(host, servicio, último refresco)` y sólo se relee al cambiar la selección o al refrescar: **119,7 ms → 0,07 ms** por repintado
- **Los grupos colapsados se volvían a expandir solos**: `refresh_data()` re-expandía *todos* los grupos en cada ciclo, así que colapsar uno se deshacía a los ≤10 s. Ahora se respeta lo que el usuario colapsó; sólo se auto-expanden los grupos que aparecieron desde el ciclo anterior. De paso, `svc_sel` se acota al refrescar si el host perdió servicios
- Verificado también que `_rrd_ds_count()` devolviendo `-1` (v3.6.7) **no** cambió el comportamiento de `graph_png`: cae en la misma rama que el `0` anterior

**Release**
- `spong.__version__`: `3.7.6`
- `setup.py`: `3.7.6`
- Paquetes: `spong-server_3.7.6-1_all.deb`, `spong-client_3.7.6-1_all.deb`
...
- En `spong top`, **`Enter` sobre un servicio** (panel derecho) abre su historial de cortes y regresos, igual que hacer clic en el servicio en la web; `←`/`h` lo cierra. Antes el historial sólo estaba en `H` (mayúscula), y al apretar `h` (minúscula) —que es "volver"— parecía que "no se veía el historial". `H` sigue funcionando como atajo (servicio si estás en el panel derecho, host si no)
- Aclaración en el README: el historial muestra sólo las **transiciones** de estado de los últimos 7 días; un servicio sin cambios de color en ese período muestra "(sin eventos en el período)"
...
**TUI: historial por servicio (cortes y regresos)**
- En el dashboard `spong top`, la tecla `H` ahora muestra el historial **del servicio seleccionado** cuando estás parado sobre él en el panel derecho: sus transiciones de estado (cuándo se cortó y cuándo volvió), como la página de servicio de la web. Si no hay un servicio seleccionado (estás navegando el panel izquierdo), muestra el historial del host completo. Repetir `H` alterna servicio → host → cerrar. Antes `H` sólo mostraba el historial del host con todos los servicios mezclados

**Release**
- `spong.__version__`: `3.7.2`
- `setup.py`: `3.7.2`
- Paquetes: `spong-server_3.7.2-1_all.deb`, `spong-client_3.7.2-1_all.deb`

### v3.7.1 — 2026-07-04

**Alias global `s` para el dashboard TUI**
- El paquete `spong-server` ahora instala un symlink `/usr/local/bin/s` → `/usr/local/spong/bin/spong-tui`. Como `/usr/local/bin` está en el PATH, alcanza con tipear `s` desde cualquier directorio para abrir el dashboard en terminal. dpkg trackea el symlink y lo elimina al desinstalar

**Release**
- `spong.__version__`: `3.7.1`
- `setup.py`: `3.7.1`
- Paquetes: `spong-server_3.7.1-1_all.deb`, `spong-client_3.7.1-1_all.deb`
